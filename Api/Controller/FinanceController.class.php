...
class FinanceController extends CommonController {
    ...
    public function get_user_coin(){
		$userid = session('userid');		
		$zichan =  M('user_coin')->where(array('userid'=>$userid)->find();
		$shizhi = M('config')->where('id=1')->getField('price');
		$zichan['shizhi'] = $shizhi;
		//昨日挖矿收益
    ...
    public function myzr()
    {
        ...
        if(!$wallet['lthb']){
            ...
            if(!$client){
                echo ajax_return(0,'连接钱包服务器失败');exit;
            }else{
                //从服务器上获取当前手机号的钱包地址
                ...
                if (!is_array($qianbao_addr)) {
                    ...
                    if (!$qianbao_ad) {
                        echo ajax_return(0,'生成钱包地址失败');exit;
                    }else {
                        $qianbao = $qianbao_ad;
        ...
        echo ajax_return(1,'成功',array('qianbao'=>$qianbao));
    }
    ...
    public function myzrdetail()
    {
        ...
        if($res){
			...
			echo ajax_return(0,'没有更多记录');
		}
    }
    /**
     * 转入记录
     */
    public function myzrdetail_more()
    {
        $userid = session('userid');
        $p = I('param.p');
	...
	public function myzc_wallet_del(){
		$id = I('post.id');
		$userid = session('userid');
	...
	*/
	public function zcwallet(){
		...
		if($info){
			...
			echo ajax_return(0,'没有找到相关信息');
		}
	}
    /**
     *添加我的转出钱包地址
     */
    public function myzc_wallet()
    {
        $name = I('post.name');
     ...
     * 转出记录
     */
    public function myzcdetail_more()
    {
        $userid = session('userid');
        $p = I('param.p',1);
        $list = 5;
     ...
     * 会员转账记录
     */
    public function transferlog_more()
    {
        $userid = session('userid');
        $p = I('param.p',1);
        $list = 5;
